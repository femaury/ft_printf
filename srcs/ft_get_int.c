...
static int	ft_set_extra(char *nb, t_format fstr)
{
	...
	if ((FLAGS & (F_PLUS | F_SPACE | F_HASH)) || nb[0] == '-')
	{
		...
		if ((FLAGS & F_HASH) && nb[0] != '0')
			if (!(TYPE == 'o') && !(TYPE == 'O'))
				extra = 2;
		return (PREC > 0 && PREC > WIDTH ? (0) : (extra));
	}
	return (0);
}
...
static void	ft_extend2(t_buffer *buff, char *nb, size_t len, t_format fstr)
{
	...
	{
		if (PREC < WIDTH - extra)
			ft_pad_buffer(buff, ' ', WIDTH - PREC - extra);
		ft_prepend(buff, nb, len, fstr);
		ft_pad_buffer(buff, (PREC > len ? '0' : ' '), (PREC < WIDTH - extra
					? PREC : WIDTH - extra) - len);
		ft_fill_buffer(buff, nb + (nb[0] == '-' ? 1 : 0), len);
	}
	else
	{
		ft_pad_buffer(buff, ' ', WIDTH - len - extra);
		ft_prepend(buff, nb, len, fstr);
		ft_fill_buffer(buff, nb + (nb[0] == '-' ? 1 : 0), len);
	}
}
	...
	if (WIDTH > len)
	{
		...
		if (FLAGS & F_MINUS)
		{
			...
			if (PREC > len && PREC < WIDTH)
				ft_pad_buffer(buff, '0', PREC - len);
			ft_fill_buffer(buff, nb + (nb[0] == '-' ? 1 : 0), len);
			ft_pad_buffer(buff, ' ', WIDTH - extra - (PREC > len ? PREC : len));
		}
		else
